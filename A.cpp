...
/*int main () {
	...
	std::cout << "ASCII message: " << (char)i1 << (char)i2 << chr)i3 << (char)i4 << (char)i5; // casting
...
/*int main() {
	...
	// else if ((baseInput == heightInput && heightInput != widthInput) || (baseInput != heightInput && heightInput == widthInput) || (baseInput == wdthnput && widthInput != heightInput)) {
...
/*int main() {
	...
	int num4 = 10;
	std::cout << (num4+=1) << std::endl; // num = nm4 + 1 = 11
...
/*int main () {
	...
	if (input2 == 0)
	{
		std::cout << "bruh your numbe is 0 << std::endl;
	}
	else
	{
...
/*int main () {
	...
	switch (operationSign) {
		...
		case '%': 
				...
				isNu2Int = num2 == static_cast<int>(num2)); // if the num2 is .0 then true
...
/*int main() {
	...
	if (input == 0) {
		std::cout << "bruh your number is ." << std:endl;
	} else {
...
/*int main() {
	...
	do { //still executes at least once even if the conditon is fals
		std::cout << "PIN: ";
		std::cin >> inputPin;
...
/*int main() {
	...
	std::cout << "Your average grade is: " << sum/gradeNumbers << st::endl;
...
void introduceMe(std::string name, int age, int salary, std::tring city) {
	td::cout << "My name is " << name << std::endl;
...
double su(int, double); //no issue as long as first argument passed is an int and second argument is a double
...
int main() {
	...
	do {
		...
		case 1:
			...
			sd::cout << "Press Enter to continue..." << std::endl;
			break;
		case 2:
		...
		std::cin.ignore(); // ignores the enter after inputting so the "press enter to ontinue" works proprly.
		getchar();
	} while (option != 4);
...
/*void menuSettings() {
	...
	case 3:
		...
		{
		std::cout << "bro, ubroke, balance hasn't changed." << std::endl;
		}
		break;
	default:
...
int main() {
	...
	swapVar(num1, num2); // the values of num1 and num2 are passed asarguments
	// since wen pass by value, the called function only gets the values and use them inside itself
...
// pass by reference// we use & to denote that the function accepts addresses as parameters
...
/*class Account {
	public:
		...
		double WithdrawAmount;
		double DepositAmount;
		//std:list<std::string> Familyames;
};
...
// Functions declared inside a class are called memer functions
// by defaul functions and variables are private when declared inside a class thus only accessible by the class itself
...
/   someFunc2(str); // s isstr as it was passed by reference, str is also modified
