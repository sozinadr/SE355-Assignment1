...
/*int main() {
	...
	// else if ((baseInput == heightInput && heightInput != widthInput) || (baseInput != heightInput && heightInput == widthInput) || (baseInput == wdthInput && widthInput != heightInput)) {
...
/*int main() {
	...
	int num4 = 10;
	std::cout << (num4+=1) << std::endl; // num = num4 + 1 = 11
...
/*int main() {
	...
	do { //still executes at least once even if the conditon is false
		std::cout << "PIN: ";
		std::cin >> inputPin;
...
int main() {
	...
	do {
		...
		std::cin.ignore(); // ignores the enter after inputting so the "press enter to ontinue" works properly.
		getchar();
	} while (option != 4);
